...
function getOAuthStartUrl(shopOrigin, nonce) {
  ...
  return "https://".concat(shopOrigin, "/admin/oauth/authorize?client_id=").concat(process.env.SHOPIFY_API_PUBLIC_KEY, "&scope=").concat(scopes, "&redirect_uri=").concat(process.env.SHOPIFY_AUTH_CALLBACK_URL, "&state=").concat(nonce);
}